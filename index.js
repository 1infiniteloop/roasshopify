...
const { logroupby, lokeyby, louniqby, lofilter, pipeLog, loorderby, lomap } = require("helpers");
...
const ipEvents = curry((version, ip, roas_user_id) => {
    ...
    return from(getDocs(q)).pipe(rxmap(Shopify.utilities.queryDocs));
});
...
const Shopify = {
    utilities: {
        ...
        date_start_end_timestamps: (
            ...
            start: moment(Shopify.utilities.date_pacific_time(start, timezone)).add(1, "days").startOf("day").valueOf(),
            end: moment(Shopify.utilities.date_pacific_time(end, timezone)).add(1, "days").endOf("day").valueOf(),
        }),
    ...
    orders: {
        get: ({ user_id, date }) => {
            let func_name = "Shopify:orders:get";
            console.log(func_name);
                ...
                let pagination_loop = async (startAt, iteration = 0) => {
                    if (startAt == false) {
                        ...
                        let db_query = query(
                            ...
                            where("created_at_unix_timestamp", ">", Number(start_timestamp)),
                            where("created_at_unix_timestamp", "<", Number(end_timestamp)),
                            orderBy("created_at_unix_timestamp"),
                            limit(1000)
                        );

                        let snapshot = await getDocs(db_query);
        ...
        without_refunds: (orders) => {
            let func_name = "Shopify:orders:without_refunds";
            console.log(func_name);

            if (!orders) return throwError(`error:${func_name}:no user_id`);
            return rxof(orders).pipe(rxmap(lofilter((item) => isEmpty(item.refunds))), defaultIfEmpty([]));
        },
    },
    ...
    order: {
        cart: {
            items: (order) => {
                let func_name = "Shopify:order:cart:items";
                console.log(func_name);

                return pipe(
                    get("line_items"),
                    mod(all)(({ price, name }) => ({ price: Number(price), name }))
                )(order);
            },
        },
        ...
        stats: {
            get: (order) => {
                let func_name = "Shopify:order:stats:get";
                console.log(func_name);

                let cart = Shopify.order.cart.items(order);
        ...
        ads: {
            ids: (order) => {
                let func_name = `Shopify:order:ads:get`;
                console.log(func_name);

                return Shopify.order.events.get(order).pipe(
                    concatMap(identity),
                    rxmap((event) => ({
        ...
        events: {
            get: (order) => {
                let func_name = `Shopify:order:events:get`;
                console.log(func_name);

                let ipv4_events = Events.user.get.ipv4({ ip: order.browser_ip, roas_user_id: order.roas_user_id });
                let ipv6_events = Events.user.get.ipv6({ ip: order.browser_ip, roas_user_id: order.roas_user_id });
    ...
    report: {
        get: ({ user_id, date, fb_ad_account_id }) => {
            let func_name = `Shopify:report:get`;
            console.log(func_name);

            let orders = Shopify.orders.get({ user_id, date }).pipe(
                concatMap(Shopify.orders.without_refunds),
                rxmap(logroupby("browser_ip")),
                rxmap(values),
                rxmap(flatten),
                concatMap(identity),
                rxmap((order) => ({
                    ...order,
                    cart: Shopify.order.cart.items(order),
                    stats: Shopify.order.stats.get(order),
                })),
                rxmap(of),
...
//         Shopify.report.get(payload).subscribe((result) => {
//             console.log("result");
//             pipeLog(result);
//         });
//     });
